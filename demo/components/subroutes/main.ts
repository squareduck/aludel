...
const mainTemplate = createTemplate({
    ...
    render: ({ model, actions, link, navigate, outlet }) => {
        return [
            m('h1', 'Subroutes'),
            m(
                'span',
                "Route action has access to local model of route's component",
            ),
            m(
                'span',
                'Components have access to "navigate" function and "locations" list',
            ),
            ...
            m(
                'button',
                { onclick: navigate('SubroutePerson', { name: model.name }) },
                'Go to ' + model.name + "'s page",
            ),
            m(
                'button',
                {
                    onclick: navigate('SubroutePersonOutlet', {
                        name: model.name,
                    }),
                },
                'Show ' + model.name + "'s page here",
            ),
            outlet(),
        ]
    },
