    ...
    render: ({model, action}) => {
        return h('div', {}, [
            ...
            h('button', {onclick: () => action.increment(1)}, 'Increment')
                ...
                p('Special action $init can be defined on any template. It will be called only when component is getting instantiated.'),
                p('If some fields are omitted from returned Local Model they will not be touched during sychronization. This is useful for async actions making sure we do not overwrite parts of Local Model with outdated values.'),
